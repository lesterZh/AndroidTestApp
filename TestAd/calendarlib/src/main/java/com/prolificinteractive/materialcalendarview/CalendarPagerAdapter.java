...
abstract class CalendarPagerAdapter<V extends CalendarPagerView> extends PagerAdapter {
    ...
    private List<CalendarDay> markedDates = new ArrayList<>();
     ...
     */
    public void setDateMarked(CalendarDay day, boolean selected) {
        if (selected) {
//            Log.i("ZHT", "setMarkedDate selected 3");
            if (!markedDates.contains(day)) {
                markedDates.add(day);
                invalidateMarkedDates();
            }
        } else {
            Log.i("ZHT", "setMarkedDate unselected 3");
            if (markedDates.contains(day)) {
                markedDates.remove(day);
                invalidateMarkedDates();
            }
        }
    }
     ...
     * Created by zhangHaiTao on 2016/5/17.
     */
    private void invalidateMarkedDates() {
//        Log.i("ZHT", "setMarkedDate 4 s");
//        Log.i("ZHT", "vs"+currentViews.size());
        for (V pagerView : currentViews) {
            Log.i("ZHT", "setMarkedDate 4 in");
            pagerView.setMarkedDates(markedDates);
        }
    }
